...
# tweet text containing seed hashtags
#time cut -f2,4 $user_tweets | code/filter_bigfile.py __user.tweetIds.with.top.conf.hashtags > __tweets.with.top.hashtags 2> __log.filter.tweets
...
rm __*
#join -t$'\t' -a1 -e "NA" -1 1 -2 1 -o'0 1.2 2.2' __user.potential.hashtags.by.frequency  __hashtag__#rand_freq10 | sort -t$'\t' -k2,2nr | awk -F"\t" '{a[NR] = $0}END{threshold=0.05*NR; for(i=1; i<= threshold; i++) print a[i]}' | grep -v '^NA' |  grep  $'NA' > output_preprocessing/research.related.hashtags2

####################################################
### APPROACH ONE: FREQUENCY BASED
####################################################
### IDEA: those hashtags used by scientists vs those
### used by random users can be done per user/ per tweet
##
###--------------------------------------------------
### count Per tweet
##
### Top conference hashtags are relevant!
### top 4 hashtags for each conference
##cut -f1,3 $conf_hashtags | grep '^[0-9]' | tr 'A-Z' 'a-z' | sort -S5G | uniq -c | sed 's/ *//; s/ /\t/' |sort -t$'\t' -k2,2 -k1,1nr |awk '{if (seen[$2]++ < 4) print $0}' |cut -f3 | sort -u > hashtags.seedconfs
##
###--------------------------------------------------
### Generate hashtags from potential researchers - positive
##cut -f3 $user_hashtags | tr 'A-Z' 'a-z' | sort -S5G | uniq -c | sed 's/ *//; s/ /\t/' | awk -F"\t" '{if ($1 > 5) print $2"\t"$1}' | sort -S3G -t$'\t' -k1,1 > __hashtag__#positive
##
### Generate hashtags from random users - negative
##cut -f3 $random_hashtags | tr 'A-Z' 'a-z' | sort -S5G | uniq -c | sed 's/ *//; s/ /\t/' | awk -F"\t" '{if ($1 > 5) print $2"\t"$1}' | sort -t$'\t' -k2,2nr | awk -F"\t" '{a[NR] = $0}END{threshold=0.1*NR; for(i=1; i<= threshold; i++) print a[i]}' |sort -S3G -t$'\t' -k1,1 > __hashtag__#negative
##
### Generate sorted list of academically relevant hashtags by frequency
##join -t$'\t' -a1 -e "NA" -1 1 -2 1 -o'0 1.2 2.2' __hashtag__#positive __hashtag__#negative |  sort -t$'\t' -k2,2nr |grep 'NA' | awk -F"\t" '{a[NR] = $0}END{threshold=0.05*NR; for(i=1;i<=NR;i++) if (i <= threshold) print a[i]}' | grep -v '^NA' > hashtag.freq.pertweet
##
###--------------------------------------------------
### count Per user
### Generate hashtags from potential researchers - positive
##cut -f1,3 $user_hashtags | awk -F"\t" '!seen[$0]++{print $2}' | tr 'A-Z' 'a-z' | sort -S5G | uniq -c | sed 's/ *//; s/ /\t/' | awk -F"\t" '{if ($1 > 2) print $2"\t"$1}' | sort -S3G -t$'\t' -k1,1 > __hashtag__#positive
##
### Generate hashtags from random users - negative
##cut -f1,3 $random_hashtags | awk -F"\t" '!seen[$0]++{print $2}' | tr 'A-Z' 'a-z' | sort -S5G | uniq -c | sed 's/ *//; s/ /\t/' | awk -F"\t" '{if ($1 > 2) print $2"\t"$1}' | sort -t$'\t' -k2,2nr | awk -F"\t" '{a[NR] = $0}END{threshold=0.1*NR; for(i=1; i<= threshold; i++) print a[i]}' |sort -S3G -t$'\t' -k1,1 > __hashtag__#negative
##
##join -t$'\t' -a1 -e"NA" -1 1 -2 1 -o'0 1.2 2.2' __hashtag__#positive __hashtag__#negative |sort -t$'\t' -k2,2nr |grep 'NA' | awk -F"\t" '{a[NR] = $0}END{threshold=0.01*NR; for(i=1; i<= threshold; i++) print a[i]}' | grep -v '^NA' > hashtag.freq.peruser
##
####################################################
### APPROACH TWO: CO-OCCURRENCE BASED
####################################################
### Unlike political hashtags, academic hashtags do not apppear (co-occur) in the same tweet.
### cut -f2,3 users/user.hashtags |tr 'A-Z' 'a-z' | awk -F"\t" '!seen[$0]{print $0}' | code/compute_cooccurrence.py seed.hashtags > hashtag.cooccurrence
##
