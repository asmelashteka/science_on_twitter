...
then
   ...
   echo "usage: ./generate_profile_features.sh <user info> <conf info> <random info> <user extended url>"
   exit 1
fi
...
cat __unigram__#rand | sort -S4G -t$'\t' -k2,2nr | awk -F"\t" '{a[NR] = $0}END{threshold=0.05*NR; for(i=1; i <= threshold; i++) print a[i]}' | sort -S4G -t$'\t' -k1,1 > __unigram__#rand_top1
...
cat __unigram__#rand | sort -S4G -t$'\t' -k2,2nr | awk -F"\t" '{a[NR] = $0}END{threshold=0.05*NR; for(i=1; i <= threshold; i++) print a[i]}' | sort -S4G -t$'\t' -k1,1 > __unigram__#rand_top10


#################################
# Trying out different thresholds

join -t$'\t' -a1 -e "NA" -1 1 -2 1 -o'0 1.2 2.2' __unigram__#positive __unigram__#rand_top1 | join -t$'\t' -a1 -e "NA" -1 1 -2 1 - __unigram__#conf -o '1.1 1.2 1.3 2.2' | sort -t$'\t' -k2,2nr | awk -F"\t" '{a[NR] = $0}END{threshold=0.10*NR; for(i=1; i<= threshold; i++) print a[i]}' | grep -v '^NA' |  grep  $'NA\tNA' > __unigram.features_top1

join -t$'\t' -a1 -e "NA" -1 1 -2 1 -o'0 1.2 2.2' __unigram__#positive __unigram__#rand_top5 | join -t$'\t' -a1 -e "NA" -1 1 -2 1 - __unigram__#conf -o '1.1 1.2 1.3 2.2' | sort -t$'\t' -k2,2nr | awk -F"\t" '{a[NR] = $0}END{threshold=0.10*NR; for(i=1; i<= threshold; i++) print a[i]}' | grep -v '^NA' |  grep  $'NA\tNA' > __unigram.features_top5

join -t$'\t' -a1 -e "NA" -1 1 -2 1 -o'0 1.2 2.2' __unigram__#positive __unigram__#rand_top10 | join -t$'\t' -a1 -e "NA" -1 1 -2 1 - __unigram__#conf -o '1.1 1.2 1.3 2.2' | sort -t$'\t' -k2,2nr | awk -F"\t" '{a[NR] = $0}END{threshold=0.10*NR; for(i=1; i<= threshold; i++) print a[i]}' | grep -v '^NA' |  grep  $'NA\tNA' > __unigram.features_top10



# Generate sorted list tokens in positive and not in negative by frequency
cat __unigram__#conf __unigram__#rand |sort -t$'\t' -k1,1 -k2,2nr | awk -F"\t" '!seen[$0]++{print $0}' > __unigram__#negative
join -t$'\t' -a1 -e "NA" -1 1 -2 1 -o'0 1.2 2.2' __unigram__#positive __unigram__#negative | sort -t$'\t' -k2,2nr |grep 'NA' | awk -F"\t" '{a[NR] = $0}END{threshold=0.1*NR; for(i=1; i<= threshold; i++) print a[i]}' | grep -v '^NA'> __unigram.features 

cat __unigram.features > profile.feature.vector 

join -t$'\t' -a1 -e "NA" -1 1 -2 1 -o'0 1.2 2.2' __unigram__#positive __unigram__#rand_top1 | join -t$'\t' -a1 -e "NA" -1 1 -2 1 - __unigram__#conf -o '1.1 1.2 1.3 2.2' | sort -t$'\t' -k2,2nr | awk -F"\t" '{a[NR] = $0}END{threshold=0.1*NR; for(i=1; i<= threshold; i++) print a[i]}' | grep -v '^NA'> __unigram.features_top1
...
cut -f3 $user_url | ./parse_urls.py > __url.parts
